...
workers = 8
epochs  = 20
batch_size = 64#torch.cuda.device_count()*32 # Change if out of cuda 
base_lr = 0.0001
momentum = 0.09
weight_decay = 1e-4
print_freq = 10
...
def main(meta,check,anal):
    global args, best_prec1, weight_decay, momentum, train_txt, valid_txt, test_txt
    ...
    test_txt = os.path.join(CHECKPOINTS_PATH,'test_%s_%s_%s' % (meta, check, anal))
    ...
    dataVal   = ITrackerData(split='val',imSize=imSize,META_NAME='metadata_%s' % meta,DATASET_PATH=DATASET_PATH,METADATA_PATH=DATASET_PATH)
    dataTest  = ITrackerData(split='test',imSize=imSize,META_NAME='metadata_%s' % meta,DATASET_PATH=DATASET_PATH,METADATA_PATH=DATASET_PATH)
    ...
    # reset learning rate to current epoch
    for epoch in range(0, epoch):
        adjust_learning_rate(optimizer, epoch)
    
    # get train, validation, and test errors    
...
def forward(imFace,imEyeL,imEyeR,faceGrid,model):
...
def train(train_loader, model, criterion,optimizer, epoch):
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(train_loader):
        ...
        # compute output
        output = forward(imFace,imEyeL,imEyeR,faceGrid,model)
...
def validate(loader, model, criterion, epoch, valortest):
    global count_test
    batch_time = AverageMeter()
    data_time  = AverageMeter()
    losses     = AverageMeter()
    ...
    oIndex = 0
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(loader):
        # measure data loading time
        data_time.update(time.time() - end)
        ...
        # validation or test loss
        lossLin = output - gaze
        lossLin = torch.sum(torch.mul(lossLin,lossLin),1)
