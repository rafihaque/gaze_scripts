...
rawDataPath="/labs/cliffordlab/data/ipad_art_gaze/EHAS/server_scripts/Retrain_CNN_Pipeline/GazeCaptureMIT/"
...
#This for-block is for preprocessesing (before pytorch) and can use pc resources (batch size higher)
for sub in "${subjectArray[@]}"
  do
    #Uncomment block below to unzip, faceDetect, Crops/Grids
    #echo "$sub""/"*xyTorch_MIT_ipad_scratch.mat
    filePath="$rawDataPath"$sub"/"*xyTorch_MIT_ipad_scratch.mat
    if [ ! -e "$filePath" ]; then
    echo "Sending $sub files to get processed"
   	#processSub "$sub" "$workDir" &
	#bash "$workDir"preProcessSub.sh "$sub" "$workDir" &
	jobCount=$((jobCount+1))
